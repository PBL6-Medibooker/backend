...
Doctor_Schema.statics.add_Doctor = async function (
  email,
  password,
  username,
  phone,
...
Doctor_Schema.statics.Is_Time_Overlap = async function (
  new_time,
  account_Id,
  excluded_time = {}
) {
  const account_active_hours = await this.findById(account_Id, {
    active_hours: 1,
  });
  ...
  for (let existing_Time of existing_Times) {
    if (
      existing_Time.day !== new_time.day ||
      existing_Time.hour_type !== new_time.hour_type
    ) {
