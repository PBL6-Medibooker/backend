...
class user_Controller {
  ...
  acc_Login = async (req, res) => {
    ...
    try {
      ...
      if (acc instanceof Doctor && acc.is_doc === "1") {
    ...
    try {
      ...
      const accounts_With_Png_Images = accounts.map((account) => {
        ...
        if (
          ...
          Buffer.isBuffer(accountObject.profile_image)
        ) {
          // Convert buffer to base64 string
          accountObject.profile_image = `data:image/png;base64,${accountObject.profile_image.toString(
            "base64"
          )}`;
        }
  ...
  get_AccountById = async (req, res) => {
    try {
      // get id
      const docId = req.params.id;

      let account = await User.findById(docId);
      if (!mongoose.Types.ObjectId.isValid(docId)) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid doc  ID format" });
      }
  ...
  update_Doctor_Info = async (req, res) => {
    try {
      ...
      console.log(speciality, region, bio);
      ...
      if (speciality) account.speciality_id = speciality;
      if (region) account.region_id = region;
